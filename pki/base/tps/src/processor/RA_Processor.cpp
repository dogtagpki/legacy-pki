    ...
    char *applet_version, char *keyVersion)
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        RA::Debug("RA_Processor::UpdateTokenRecoveredCert", "Key type of current cert: %d", pk_p->keyType);

        if (pk_p->keyType == ecKey) {
            RA::Debug("RA_Processor::UpdateTokenRecoveredCert", "Recovering ECC certificate.");            
            isECC = true;
        } 
        ...
        if (!isECC) {
            ...
            int publicValueLen = pk_p->u.ec.publicValue.len - 1;

            keysize =RA::getECCKeySize(&pk_p->u.ec.DEREncodedParams);
            RA::Debug("RA_Processor::UpdateTokenRecoveredCert", "Key size of current cert: %d ", keysize);
          
        }

        if ( keysize == 0) {
            RA::Debug("RA_Processor::UpdateTokenRecoveredCert",
                         "Illegal key size encountered");
                          status = STATUS_ERROR_RECOVERY_FAILED;
                          PR_snprintf(audit_msg, 512, "Key Recovery failed. Illegal key size encountered.");
            status = STATUS_ERROR_RECOVERY_FAILED;
            goto loser;
        }
	 ...
	 * The keyIdentifier is composed of the 160-bit SHA-1 hash of the
	    ...
	    //RA::DebugBuffer("cfu debug"," private key =",decodeKey);
            ...
            Buffer(1, 0x01) + // encryption
			    Buffer(1, keytype)+ // keytype is RSAPKCS8Pair
        ...
        /* url decode the wrapped kek session key and keycheck*/
        {
            ...
            if (isECC) {
                algorithm = RA::getECCAlg(&pk_p->u.ec.DEREncodedParams);

                if (algorithm == 0) { // Unknown or error alg
                     PR_snprintf(audit_msg, 512, "ProcessRecovery: store keys in token failed, unknown ecc alrogithm detected.");
                     delete decodeKey;
